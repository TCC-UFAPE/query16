...
def run_task_1_and_2(vulnerabilities):
    ...
    print("   [FILTRO ATIVO] Processando apenas projeto django")
...
if __name__ == "__main__":
    ...
    if all_vulnerabilities and all_tags_map:
        # Filtrar apenas vulnerabilidades do django
        httpd_vulns = [v for v in all_vulnerabilities if v.get('project_name', '').lower() == 'django']
        print(f"\n[FILTRO] {len(httpd_vulns)} vulnerabilidades do django de {len(all_vulnerabilities)} totais")
        
        if not httpd_vulns:
            print("\n[ERRO] Nenhuma vulnerabilidade do django encontrada.")
        else:
            # Executar tarefas da query7 apenas com django
            run_task_1_and_2(httpd_vulns)
            time.sleep(1)
            run_task_3_and_4(httpd_vulns, all_tags_map)
            time.sleep(1)
            
            # Executar análise combinada (query7 + query10) apenas com django
